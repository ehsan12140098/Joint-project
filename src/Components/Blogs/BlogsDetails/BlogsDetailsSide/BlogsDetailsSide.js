...
import BlogsSideSection from 'Components/Blogs/BlogsSideSection/BlogsSideSection';
...
export default function BlogsDetailsSide() {
  const isLgOrBigger = useIsDesktopDisplay();
  return (
    <div
      style={{
        display: 'flex',
        flexDirection: isLgOrBigger ? 'column' : 'column-reverse',
      }}
    >
      <BlogsSideSection
        isDense
        title="پر بازدید ترین مطالب"
      ...
      >
        {tourDetail.suggestedTours.map(tour => (
          <Grid item xs={12} sm={6} lg={12} key={tour.id}>
